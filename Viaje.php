...
class Viaje{
    ...
    public function __construct($importe,$tramos,$codigo, $desti, $capacidad,$colPasajeros,$responsable ){
    ...
    public function setTramos($nuevoTramos)
    {
    $this->tramos = $nuevoTramos;
    }
...
public function venderPasaje($pasajero){
    ...
    if($this->haypasajesdisponible()){
        $salidaImporte=$this->getimporte();
        if($this->getTramos()=="ida y vuelta"){
            $salidaImporte=$salidaImporte*2;
            
        }
    }
    return $salidaImporte;
}
public function haypasajesdisponible(){
    //var_dump($this->getColeccionPasajeros());
    $A = count($this->getColeccionPasajeros());
    if ($A <= $this->cantPasajeros){
        $hayPasajes = true;
    }else{
        $hayPasajes = false;
    }
